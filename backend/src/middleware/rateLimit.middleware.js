...
export const apiRateLimiter = rateLimit({
  ...
  max: process.env.NODE_ENV === 'test' ? 10000 : 200, // Limit each IP to 200 requests per windowMs in dev/prod
...
export const publicBookingRateLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: process.env.NODE_ENV === 'test' ? 1000 : 60, // 60 booking/cancel requests per 15 mins
  standardHeaders: true,
  legacyHeaders: false,
  handler: (req, res) => {
    res.status(429).json({
